        ...
        <div className={style.wrapper}>
            ...
            <div className={style.upcontent}>
                <div className={style.content1} >
                    <p className={style.got}>Time to Splurge</p>
                    Spend your GMILK on GOATz Upgrades, Trait Transformations, Experiences, Merch and More.
                    <br/><br/>
                    There’s always new items being added to the marketplace so stop in from time to time.
                </div>

                {props.isEnabled ? (
                        <button className={style.btn} onClick={() => toastr.info('Copied your wallet ID')}
                        style={{textTransform:"uppercase"}}>{getShortAccountId()}</button>
                    ) : (
                        <button className={style.btn} onClick={connectWallet}>CONNECT</button>
                )}
                        <div className={style.goatimg} style={{backgroundSize: "contain",backgroundPosition:"bottom"}}>
            ...
            {isLoading ? <Loader /> : <div className={style.item}>
                    ...
                    {nftList?.map((elm: any) => <div className={style['nft__grid--card']} key={elm._id}>
                        <div className={style.item__img}>
                            <img
                                src={elm?.imagePath}
                                alt={elm.title}
                                style={{ width: '295px', height: '230px', objectFit: "cover" }}
                            />
                        </div>
